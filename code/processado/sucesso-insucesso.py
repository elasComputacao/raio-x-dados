...
def pre_processamento():
    ...
    geral = geral[['id', 'tipo_matricula', 'nome_disciplina', 'situacao', 'sexo']]
...
def get_df_taxas(geral):
    ...
    df_taxas = pd.DataFrame(geral.groupby(['sexo', 'situacao', 'nome_disciplina'])['id'].count())\
                                 .reset_index().rename(columns={'id':'quantidade'})

    # Criando tabelas temporárias para cada situação
    # Aprovados
    temp_apv = pd.DataFrame(df_taxas[df_taxas.situacao == 'APROVADO'].groupby(['nome_disciplina', 'sexo'])['quantidade'].sum())\
                                    .reset_index().rename(columns={'quantidade':'aprovado'})
    # Reprovados
    temp_rep = pd.DataFrame(df_taxas[df_taxas.situacao == 'REPROVADO'].groupby(['nome_disciplina', 'sexo'])['quantidade'].sum())\
    ...
    temp_rep_falta = pd.DataFrame(df_taxas[df_taxas.situacao == 'REPROVADO POR FALTA']\
                                        .groupby(['nome_disciplina', 'sexo'])['quantidade'].sum())\
                                        .reset_index().rename(columns={'quantidade':'reprovado_falta'})

    # Merge Dataframes
    df_taxas = reduce(lambda left,right: pd.merge(left,right, on=['nome_disciplina', 'sexo'], how='outer'), \
